...
for obs in os.listdir(obs_dir):

    arfiles_list = []

    for subfolder in os.listdir(obs_dir + obs):
        if os.path.isdir(obs_dir + obs + "/" + subfolder):
            for fname in os.listdir(obs_dir + obs + "/" + subfolder):
                if ".ar" in fname:
                    arfiles_list += [obs_dir + obs + "/" + subfolder + "/" +fname]

    for arfile in arfiles_list:
